...
//Servo myservo;
//MQUnifiedsensor MQ3(GAS_PIN, AnalogMode);
...
void loop() {
  ...
  int soundValue = analogRead(soundSensor);  // Lee el valor analógico del sensor
  ...
  if (pirValue == HIGH/*gasValue > 0 || pirValue == HIGH || flameValue == HIGH*/) {
    ...
    servoMotor.write(100);     // sets the servo position to 0 degrees
    delay(1000);          // waits for 1 second
    servoMotor.write(0);   // sets the servo position to 180 degrees
  ...
  char msg[64];
  snprintf(msg, 64, "%d,%d,%d,%.2f,%.2f", soundValue, pirValue, flameValue, t, h);
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == topicServo) {
    ...
    if (messageTemp == "true") {
      ...
      tone(BUZZER_PIN, 100); // Hacemos sonar el buzzer a una frecuencia de 1000 Hz
      // client.publish(topicServo, "true");
    } else if (messageTemp == "false") {
